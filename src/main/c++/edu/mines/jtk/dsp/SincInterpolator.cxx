...
*/           
/******************************************************************************/
/* CREATOR:     Tiancheng Song                       DATE: Dec 2014
 * DESCRIPTION: Include file to declare the following classes:
 *                       -   SincInterpolator, KaiserWindow
 * Member Functions:
 *
 * SincInterpolator::SincInterpolator 
 * SincInterpolator::~SincInterpolator
 * SincInterpolator::fromErrorAndLength
 * SincInterpolator::fromErrorAndFrequency
 * SincInterpolator::fromFrequencyAndLength
 * SincInterpolator::getMaximumError
 * SincInterpolator::getMaximumFrequency
 * SincInterpolator::getMaximumLength
 * SincInterpolator::getTableBytes
 * SincInterpolator::getExtrapolation
 * SincInterpolator::setExtrapolation
 * SincInterpolator::interpolate
 * SincInterpolator::interpolateComplex
 * SincInterpolator::getTable
 * SincInterpolator::writeOutput
 * SincInterpolator::cleanup

 * KaiserWindow::fromErrorAndWidth
 * KaiserWindow::fromErrorAndLength
 * KaiserWindow::fromWidthAndLength
 * KaiserWindow::evaluate
...
#include <cmath>
   ...
   */
  double SincInterpolator::EWIN_FRAC = 0.9;
  int SincInterpolator::NTAB_MAX = 16385;
  SincInterpolator SincInterpolator::fromErrorAndLength (
    double emax, int lmax)
  {
    return SincInterpolator(emax,0.0,lmax);
  }
   ...
   * @return the sinc interpolator.
   */
  SincInterpolator SincInterpolator::fromErrorAndFrequency(
    double emax, double fmax)
  {
    return SincInterpolator(emax,fmax,0);
  }
   ...
   * @return the sinc interpolator.
   */
  SincInterpolator SincInterpolator::fromFrequencyAndLength(
    double fmax, int lmax)
  {
    return SincInterpolator(0.0,fmax,lmax);
  }
   ...
   */
  SincInterpolator::SincInterpolator():_table() {
    _extrap = ZERO;
    init(0.0,0.3,8);
  }

  SincInterpolator::SincInterpolator(double emax,double fmax,int lmax){
    init(emax, fmax,lmax);
  }
  /**
   ...
   */
  double SincInterpolator::getMaximumError() {
   ...
   */
  double SincInterpolator::getMaximumFrequency() {
   ...
   */
  int SincInterpolator::getMaximumLength() {
   ...
   */
  long SincInterpolator::getTableBytes() {
    long nbytes = 4L;
    nbytes *= _table.lsinc;
    nbytes *= _table.nsinc;
    return nbytes;
   ...
   */
  void SincInterpolator::setExtrapolation(Extrapolation extrap) {
    _extrap = extrap;
  }
   ...
   */
  void SincInterpolator::interpolate(
   ...
   * @return interpolated value y(x).
   */
  float SincInterpolator::interpolate(Sampling sxu, vector<float>& yu, double xi) {
    assert(sxu.isUniform());
  ...
  float SincInterpolator::interpolate(
    ...
    vector<vector<float> >& yu, double x1i, double x2i)
  {
    assert(sx1u.isUniform());
    assert(sx2u.isUniform());
    return interpolate(
   ...
   */
  void SincInterpolator::interpolateComplex(
    int nxu, double dxu, double fxu, vector<float>& yu, 
    int nxi, double dxi, double fxi, vector<float>& yi)
  {
    double xscale = 1.0/dxu;
   ...
   */
  void SincInterpolator::accumulate(
    double xa, float ya,
    int nxu, double dxu, double fxu, vector<float>& yu) 
  {
    double xscale = 1.0/dxu;
   ...
   */
  vector<vector<float> >SincInterpolator::getTable() {
    vector<vector<float> >temp = _table.asinc;
    return temp;
  }
   ...
   */
  void SincInterpolator::init(double emax,double fmax,int lmax) {
    assert(emax==0.0 && fmax!=0.0 && lmax!=0 ||
   ...
   */
  SincInterpolator::Table SincInterpolator::makeTable(Design& design) {
    ...
    table.design = Design(emax,fmax,lmax); 
    return table;
  }
   ...
   */
  SincInterpolator::Table  SincInterpolator::makeTable(int nsinc, int lsinc, KaiserWindow& kwin) {
    vector<vector<float> >asinc(nsinc,vector <float>(lsinc));// = new float[nsinc][lsinc];
    int nsincm1 = nsinc-1;
    ...
    SincInterpolator::Table table; //new SincInterpolator::Table();
    table.lsinc = lsinc;
    table.nsinc = nsinc;
  ...
  double SincInterpolator::sinc(double x) {
   ...
   */
  SincInterpolator::Table  SincInterpolator::getTable(double emax, double fmax, int lmax) {
    Design design =  Design(emax,fmax,lmax);
      Table  table = makeTable(design);
      return table;
    //}
  }
    ...
    vector<float>& yu, double x)
  {
    ...
    vector<float>& asinc = _asinc[ksinc];
    ...
    } else if (_extrap==ZERO) {
      for (int isinc=0; isinc<_lsinc; ++isinc,++kyu) {
        if (0<=kyu && kyu<nxu)
          yr += yu[kyu]*asinc[isinc];
      }
    } else if (_extrap==CONSTANT) {
      for (int isinc=0; isinc<_lsinc; ++isinc,++kyu) {
    ...
    vector<vector<float> >& yu, double x1, double x2)
  {
    ...
    vector<float>& asinc1 = _asinc[ksinc1];
    vector<float>& asinc2 = _asinc[ksinc2];
    ...
    vector<vector<vector<float> > >&yu, double x1, double x2, double x3)
  {
    ...
    vector<float>& asinc1 = _asinc[ksinc1];
    vector<float>& asinc2 = _asinc[ksinc2];
    vector<float>& asinc3 = _asinc[ksinc3];
    ...
    if (ky1u>=0 && ky1u<=nx1um &&  
      ...
      for (int i3sinc=0; i3sinc<_lsinc; ++i3sinc,++ky3u) {
        ...
        vector<vector<float> >& yu3 = yu[ky3u];
        float yr2 = 0.0f;
        for (int i2sinc=0,my2u=ky2u; i2sinc<_lsinc; ++i2sinc,++my2u) {
          float asinc22 = asinc2[i2sinc];
          vector<float>& yu32 = yu3[my2u];
          float yr1 = 0.0f;
          for (int i1sinc=0,my1u=ky1u; i1sinc<_lsinc; ++i1sinc,++my1u)
