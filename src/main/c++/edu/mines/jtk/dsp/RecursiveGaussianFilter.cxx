/*! \file RecursiveGaussianFilter.cxx */

/******************************************************************************
 * C++ port from Java by Tiancheng Song
 * Please refer to the header file for complete documentation and licensing information.
 *
 * Member Functions:
 *
 *       Filter::applyN
 *       Filter::applyXN
 *       Filter::applyNX
 *       Filter::applyNXX
 *       Filter::applyXNX
 *       Filter::applyXXN

 *       RecursiveGaussianFilter::RecursiveGaussianFilter
 *       RecursiveGaussianFilter::~RecursiveGaussianFilter
 *       RecursiveGaussianFilter::apply0
 *       RecursiveGaussianFilter::apply1
 *       RecursiveGaussianFilter::apply2
 *       RecursiveGaussianFilter::apply0X
 *       RecursiveGaussianFilter::apply1X
 *       RecursiveGaussianFilter::apply2X
 *       RecursiveGaussianFilter::applyX0
 *       RecursiveGaussianFilter::applyX1
 *       RecursiveGaussianFilter::applyX2
 *       RecursiveGaussianFilter::apply0XX
 *       RecursiveGaussianFilter::apply1XX
 *       RecursiveGaussianFilter::apply2XX
 *       RecursiveGaussianFilter::applyX0X
 *       RecursiveGaussianFilter::applyX1X
 *       RecursiveGaussianFilter::applyX2X
 *       RecursiveGaussianFilter::applyXX0
 *       RecursiveGaussianFilter::applyXX1
 *       RecursiveGaussianFilter::applyXX2
 *       RecursiveGaussianFilter::apply00
 *       RecursiveGaussianFilter::apply10
 *       RecursiveGaussianFilter::apply01
 *       RecursiveGaussianFilter::apply11
 *       RecursiveGaussianFilter::apply20
 *       RecursiveGaussianFilter::apply02
 *       RecursiveGaussianFilter::apply000
 *       RecursiveGaussianFilter::apply100
 *       RecursiveGaussianFilter::apply010
 *       RecursiveGaussianFilter::apply001
 *       RecursiveGaussianFilter::apply110
 *       RecursiveGaussianFilter::apply101
 *       RecursiveGaussianFilter::apply011
 *       RecursiveGaussianFilter::apply200
 *       RecursiveGaussianFilter::apply020
 *       RecursiveGaussianFilter::apply002
 *       RecursiveGaussianFilter::checkArrays
 *       RecursiveGaussianFilter::sameArrays
 *
 *       DericheFilter::DericheFilter
 *       DericheFilter::applyN
 *       DericheFilter::applyXN
 *       DericheFilter::makeND
 *       DericheFilter::scaleN
 *
 *       VanVlietFilter::VanVlietFilter
 *			 VanVlietFilter::applyN
 *			 VanVlietFilter::applyXN
 *			 VanVlietFilter::makeG(double sigma);
 *			 VanVlietFilter::makeFilter(double b0, double b1, double b2, double a1, double a2);
 *			 VanVlietFilter::gr(int nd, Cdouble polej, const vector<Cdouble>& poles, double gain) ;
 *			 VanVlietFilter::adjustPoles(double sigma, const vector<Cdouble>& poles) ;
 *			 VanVlietFilter::computeGain(vector<Cdouble>&  poles);
 *			 VanVlietFilter::computeSigma(double sigma, const vector<Cdouble>& poles);
 ******************************************************************************
*/

#include "RecursiveGaussianFilter.h"
#include "CDouble.h"
#include <cmath>
#include <cfloat>
#include <vector>
#include <algorithm>
#include <assert.h>

/**
 * Construct a Gaussian filter with specified width and design method.
 * @param sigma the width; must not be less than 1.
 * @param method the method used to design the filter.
 */
RecursiveGaussianFilter::RecursiveGaussianFilter(double sigma, Method method)
{
  if (method == DERICHE)
  {
    _filter =  new DericheFilter(sigma);
  }
  else
  {		
    _filter =	 new VanVlietFilter(sigma);
  }	
  mySigma = sigma;
}

/**
 * Construct a Gaussian filter with specified width.
 * @param sigma the width; must not be less than 1.
 */
RecursiveGaussianFilter::RecursiveGaussianFilter(double sigma) 
{
  if (sigma < 32.0)
  {
    _filter =  new DericheFilter(sigma);
  }
  else
  {		
    _filter =	 new VanVlietFilter(sigma);
  }
  mySigma = sigma;
}

RecursiveGaussianFilter::~RecursiveGaussianFilter()
{
}

/**
  * Applies the 0th-derivative filter.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply0(const vector<float>& x, vector<float>& y) 
{
  _filter->applyN(0,x,y);
}

/**
  * Applies the 1st-derivative filter.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply1(const vector<float>& x, vector<float>& y) 
{
  _filter->applyN(1,x,y);
}

/**
  * Applies the 2nd-derivative filter.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply2(const vector<float>& x, vector<float>& y) 
{
  _filter->applyN(2,x,y);
}

/**
  * Applies the 0th-derivative filter along the 1st dimension.
  * Applies no filter along the 2nd dimension.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply0X(const vector< vector<float> >& x, vector< vector<float> >& y) 
{
  _filter->applyNX(0,x,y);
}

/**
  * Applies the 1st-derivative filter along the 1st dimension.
  * Applies no filter along the 2nd dimension.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply1X(const vector< vector<float> >& x, vector< vector<float> >& y) 
{
  _filter->applyNX(1,x,y);
}

/**
  * Applies the 2nd-derivative filter along the 1st dimension.
  * Applies no filter along the 2nd dimension.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply2X(const vector< vector<float> >& x, vector< vector<float> >& y) 
{
  _filter->applyNX(2,x,y);
}

/**
  * Applies the 0th-derivative filter along the 2nd dimension.
  * Applies no filter along the 1st dimension.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::applyX0(const vector< vector<float> >& x, vector< vector<float> >& y) 
{
  _filter->applyXN(0,x,y);
}

/**
  * Applies the 1st-derivative filter along the 2nd dimension.
  * Applies no filter along the 1st dimension.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::applyX1(const vector< vector<float> >& x, vector< vector<float> >& y) 
{
  _filter->applyXN(1,x,y);
}

/**
  * Applies the 2nd-derivative filter along the 2nd dimension.
  * Applies no filter along the 1st dimension.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::applyX2(const vector< vector<float> >& x, vector< vector<float> >& y) 
{
  _filter->applyXN(2,x,y);
}

/**
  * Applies the 0th-derivative filter along the 1st dimension.
  * Applies no filter along the 2nd or 3rd dimensions.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply0XX(const vector<vector<vector<float> > >& x, vector<vector<vector<float> > >& y) 
{
  _filter->applyNXX(0,x,y);
}

/**
  * Applies the 1st-derivative filter along the 1st dimension.
  * Applies no filter along the 2nd or 3rd dimensions.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply1XX(const vector<vector<vector<float> > >& x, vector<vector<vector<float> > >& y) 
{
  _filter->applyNXX(1,x,y);
}

/**
  * Applies the 2nd-derivative filter along the 1st dimension.
  * Applies no filter along the 2nd or 3rd dimensions.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply2XX(const vector<vector<vector<float> > >& x, vector<vector<vector<float> > >& y) 
{
  _filter->applyNXX(2,x,y);
}

/**
  * Applies the 0th-derivative filter along the 2nd dimension.
  * Applies no filter along the 1st or 3rd dimensions.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::applyX0X(const vector<vector<vector<float> > >& x, vector<vector<vector<float> > >& y) 
{
  _filter->applyXNX(0,x,y);
}

/**
  * Applies the 1st-derivative filter along the 2nd dimension.
  * Applies no filter along the 1st or 3rd dimensions.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::applyX1X(const vector<vector<vector<float> > >& x, vector<vector<vector<float> > >& y) 
{
  _filter->applyXNX(1,x,y);
}

/**
  * Applies the 2nd-derivative filter along the 2nd dimension.
  * Applies no filter along the 1st or 3rd dimensions.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::applyX2X(const vector<vector<vector<float> > >& x, vector<vector<vector<float> > >& y) 
{
  _filter->applyXNX(2,x,y);
}

/**
  * Applies the 0th-derivative filter along the 3rd dimension.
  * Applies no filter along the 1st or 2nd dimensions.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::applyXX0(const vector<vector<vector<float> > >& x, vector<vector<vector<float> > >& y) 
{
  _filter->applyXXN(0,x,y);
}

/**
  * Applies the 1st-derivative filter along the 3rd dimension.
  * Applies no filter along the 1st or 2nd dimensions.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::applyXX1(const vector<vector<vector<float> > >& x, vector<vector<vector<float> > >& y)
{
  _filter->applyXXN(1,x,y);
}

/**
  * Applies the 2nd-derivative filter along the 3rd dimension.
  * Applies no filter along the 1st or 2nd dimensions.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::applyXX2(const vector<vector<vector<float> > >& x, vector<vector<vector<float> > >& y) 
{
  _filter->applyXXN(2,x,y);
}

/**
  * Applies the 0th-derivative filter along the 1st and 2nd dimensions.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply00(const vector< vector<float> >& x, vector< vector<float> >& y)
{
  _filter->applyXN(0,x,y);
  _filter->applyNX(0,y,y);
}

/**
  * Applies the 1st-derivative filter along the 1st dimension
  * and the 0th-derivative filter along the 2nd dimension.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply10(const vector< vector<float> >& x, vector< vector<float> >& y) 
{
  _filter->applyXN(0,x,y);
  _filter->applyNX(1,y,y);
}

/**
  * Applies the 0th-derivative filter along the 1st dimension
  * and the 1st-derivative filter along the 2nd dimension.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply01(const vector< vector<float> >& x, vector< vector<float> >& y) 
{
  _filter->applyXN(1,x,y);
  _filter->applyNX(0,y,y);
}

/**
  * Applies the 1st-derivative filter along the 1st and 2nd dimensions.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply11(const vector< vector<float> >& x, vector< vector<float> >& y) 
{
  _filter->applyXN(1,x,y);
  _filter->applyNX(1,y,y);
}

/**
  * Applies the 2nd-derivative filter along the 1st dimension
  * and the 0th-derivative filter along the 2nd dimension.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply20(const vector< vector<float> >& x, vector< vector<float> >& y) 
{
  _filter->applyXN(0,x,y);
  _filter->applyNX(2,y,y);
}

/**
  * Applies the 0th-derivative filter along the 1st dimension
  * and the 2nd-derivative filter along the 2nd dimension.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply02(const vector< vector<float> >& x, vector< vector<float> >& y) 
{
  _filter->applyXN(2,x,y);
  _filter->applyNX(0,y,y);
}

/**
  * Applies the 0th-derivative filter along the 1st, 2nd and 3rd dimensions.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply000(const vector<vector<vector<float> > >& x, vector<vector<vector<float> > >& y) 
{
  _filter->applyXXN(0,x,y);
  _filter->applyXNX(0,y,y);
  _filter->applyNXX(0,y,y);
}

/**
  * Applies the 1st-derivative filter along the 1st dimension
  * and the 0th-derivative filter along the 2nd and 3rd dimensions.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply100(const vector<vector<vector<float> > >& x, vector<vector<vector<float> > >& y)
{
  _filter->applyXXN(0,x,y);
  _filter->applyXNX(0,y,y);
  _filter->applyNXX(1,y,y);
}

/**
  * Applies the 1st-derivative filter along the 2nd dimension
  * and the 0th-derivative filter along the 1st and 3rd dimensions.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply010(const vector<vector<vector<float> > >& x, vector<vector<vector<float> > >& y) 
{
  _filter->applyXXN(0,x,y);
  _filter->applyXNX(1,y,y);
  _filter->applyNXX(0,y,y);
}

/**
  * Applies the 1st-derivative filter along the 3rd dimension
  * and the 0th-derivative filter along the 1st and 2nd dimensions.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply001(const vector<vector<vector<float> > >& x, vector<vector<vector<float> > >& y) 
{
  _filter->applyXXN(1,x,y);
  _filter->applyXNX(0,y,y);
  _filter->applyNXX(0,y,y);
}

/**
  * Applies the 1st-derivative filter along the 1st and 2nd dimensions
  * and the 0th-derivative filter along the 3rd dimension.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply110(const vector<vector<vector<float> > >& x, vector<vector<vector<float> > >& y) 
{
  _filter->applyXXN(0,x,y);
  _filter->applyXNX(1,y,y);
  _filter->applyNXX(1,y,y);
}

/**
  * Applies the 1st-derivative filter along the 1st and 3rd dimensions
  * and the 0th-derivative filter along the 2nd dimension.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply101(const vector<vector<vector<float> > >& x, vector<vector<vector<float> > >& y) 
{
  _filter->applyXXN(1,x,y);
  _filter->applyXNX(0,y,y);
  _filter->applyNXX(1,y,y);
}

/**
  * Applies the 1st-derivative filter along the 2nd and 3rd dimensions
  * and the 0th-derivative filter along the 1st dimension.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply011(const vector<vector<vector<float> > >& x, vector<vector<vector<float> > >& y) 
{
  _filter->applyXXN(1,x,y);
  _filter->applyXNX(1,y,y);
  _filter->applyNXX(0,y,y);
}

/**
  * Applies the 2nd-derivative filter along the 1st dimension
  * and the 0th-derivative filter along the 2nd and 3rd dimensions.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply200(const vector<vector<vector<float> > >& x, vector<vector<vector<float> > >& y) 
{
  _filter->applyXXN(0,x,y);
  _filter->applyXNX(0,y,y);
  _filter->applyNXX(2,y,y);
}

/**
  * Applies the 2nd-derivative filter along the 2nd dimension
  * and the 0th-derivative filter along the 1st and 3rd dimensions.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply020(const vector<vector<vector<float> > >& x, vector<vector<vector<float> > >& y) 
{
  _filter->applyXXN(0,x,y);
  _filter->applyXNX(2,y,y);
  _filter->applyNXX(0,y,y);
}

/**
  * Applies the 2nd-derivative filter along the 3rd dimension
  * and the 0th-derivative filter along the 1st and 2nd dimensions.
  * @param x the filter input.
  * @param y the filter output.
  */
void RecursiveGaussianFilter::apply002(const vector<vector<vector<float> > >& x, vector<vector<vector<float> > >& y) 
{
  _filter->applyXXN(2,x,y);
  _filter->applyXNX(0,y,y);
  _filter->applyNXX(0,y,y);
}

void RecursiveGaussianFilter::checkArrays(const vector<float>& x, const vector<float>& y) 
{
  assert(x.size()== y.size());
}

void RecursiveGaussianFilter::checkArrays(const vector< vector<float> >& x, const vector< vector<float> >& y) 
{
  assert(x.size() ==y.size());
  assert(x[0].size() ==y[0].size());
}

void RecursiveGaussianFilter::checkArrays(const vector<vector<vector<float> > >& x, const vector<vector<vector<float> > >& y)
{
  assert(x.size() ==y.size());
  assert(x[0].size() ==y[0].size());
  assert(x[0][0].size() ==y[0][0].size());
}

bool RecursiveGaussianFilter::sameArrays(const vector<float>& x, const vector<float>& y) 
{
  return (&x==&y);
}

bool RecursiveGaussianFilter::sameArrays(const vector< vector<float> >& x, const vector< vector<float> >& y) 
{
  if (&x==&y) {
    return true;
  } else {
    int n2 = x.size();
    for (int i2=0; i2<n2; ++i2)
      if (x[i2]==y[i2])
        return true;
  }
  return false;
}

///////////////////////////////////////////////////////////////////////////
void Filter::applyNX(int nd, const vector< vector<float> >& x, vector< vector<float> >& y) 
{
  int m2 = y.size();
  for (int i2=0; i2<m2; ++i2)
    applyN(nd,x[i2],y[i2]);
}

void Filter::applyNXX(int nd, const vector<vector<vector<float> > >& x,  vector<vector<vector<float> > >& y) 
{
  const int m3 = y.size();
  for (int i3=0; i3<m3; ++i3)
    applyNX(nd,x[i3],y[i3]);
  /*
  Parallel.loop(m3,new Parallel.LoopInt() {
    public void compute(int i3) {
      applyNX(nd,x[i3],y[i3]);
    }
  });
  */
}

void Filter::applyXNX(int nd, const vector<vector<vector<float> > >& x,  vector<vector<vector<float> > >& y) 
{
  const int m3 = y.size();
  for (int i3=0; i3<m3; ++i3)
    applyXN(nd,x[i3],y[i3]);
  /*
  Parallel.loop(m3,new Parallel.LoopInt() {
    public void compute(int i3) {
      applyXN(nd,x[i3],y[i3]);
    }
  });
  */
}

void Filter::applyXXN(int nd, const vector<vector<vector<float> > >& x,  vector<vector<vector<float> > >& y) 
{
  RecursiveGaussianFilter::checkArrays(x,y);
  const int m3 = y.size();
  const int m2 = y[0].size();
  vector<vector<vector<float> > > tx; // = new float[m2][m3][];
  vector<vector<vector<float> > > ty; // = new float[m2][m3][];
  tx.resize(m2);
  ty.resize(m2);
  for(int i = 0 ; i < m2 ; ++i)
  {
    tx[i].resize(m3);
    ty[i].resize(m3);
  }

  for (int i3=0; i3<m3; ++i3) {
    for (int i2=0; i2<m2; ++i2) {
      tx[i2][i3] = x[i3][i2];
      ty[i2][i3] = y[i3][i2];
    }
  }
  for (int i2 = 0; i2<m2; i2++)
  {
    applyXN(nd,tx[i2],ty[i2]);
  }
  /*
  Parallel.loop(m2,new Parallel.LoopInt() {
    public void compute(int i2) {
      applyXN(nd,tx[i2],ty[i2]);
    }
  });
  */
}

///////////////////////////////////////////////////////////////////////////
double DericheFilter::a00 =  1.6797292232361107; 
double DericheFilter::a10 =  3.7348298269103580;
double DericheFilter::b00 =  1.7831906544515104;
double DericheFilter::b10 =  1.7228297663338028;
double DericheFilter::c00 = -0.6802783501806897;
double DericheFilter::c10 = -0.2598300478959625;
double DericheFilter::w00 =  0.6318113174569493; 
double DericheFilter::w10 =  1.9969276832487770;
// 1st derivative.
double DericheFilter::a01 =  0.6494024008440620;
double DericheFilter::a11 =  0.9557370760729773;
double DericheFilter::b01 =  1.5159726670750566;
double DericheFilter::b11 =  1.5267608734791140;
double DericheFilter::c01 = -0.6472105276644291; 
double DericheFilter::c11 = -4.5306923044570760;
double DericheFilter::w01 =  2.0718953658782650;
double DericheFilter::w11 =  0.6719055957689513;
// 2nd derivative.
double DericheFilter::a02 =  0.3224570510072559;
double DericheFilter::a12 = -1.7382843963561239;
double DericheFilter::b02 =  1.3138054926516880;
double DericheFilter::b12 =  1.2402181393295362;
double DericheFilter::c02 = -1.3312275593739595;
double DericheFilter::c12 =  3.6607035671974897;
double DericheFilter::w02 =  2.1656041357418863;
double DericheFilter::w12 =  0.7479888745408682;
//
double DericheFilter::a0[] = {a00,a01,a02};
double DericheFilter::a1[] = {a10,a11,a12};
double DericheFilter::b0[] = {b00,b01,b02};
double DericheFilter::b1[] = {b10,b11,b12};
double DericheFilter::c0[] = {c00,c01,c02};
double DericheFilter::c1[] = {c10,c11,c12};
double DericheFilter::w0[] = {w00,w01,w02};
double DericheFilter::w1[] = {w10,w11,w12};

DericheFilter::DericheFilter(double sigma) 
{
  makeND(sigma);
}

void DericheFilter::applyN(int nd, const vector<float>& x, vector<float>& y)
{
  assert(&x != &y);
  int m = y.size();
  float n0 = _n0[nd],  n1 = _n1[nd],  n2 = _n2[nd],  n3 = _n3[nd];
  float d1 = _d1[nd],  d2 = _d2[nd],  d3 = _d3[nd],  d4 = _d4[nd];
  float yim4 = 0.0f,  yim3 = 0.0f,  yim2 = 0.0f,  yim1 = 0.0f;
  float               xim3 = 0.0f,  xim2 = 0.0f,  xim1 = 0.0f;
  for (int i=0; i<m; ++i) {
    float xi = x[i];
    float yi = n0*xi+n1*xim1+n2*xim2+n3*xim3 -
                      d1*yim1-d2*yim2-d3*yim3-d4*yim4;
    y[i] = yi;
    yim4 = yim3;  yim3 = yim2;  yim2 = yim1;  yim1 = yi;
                  xim3 = xim2;  xim2 = xim1;  xim1 = xi;
  }
  n1 = n1-d1*n0;
  n2 = n2-d2*n0;
  n3 = n3-d3*n0;
  float n4 = -d4*n0;
  if (nd%2!=0) {
    n1 = -n1;  n2 = -n2;  n3 = -n3;  n4 = -n4;
  }
  float yip4 = 0.0f,  yip3 = 0.0f,  yip2 = 0.0f,  yip1 = 0.0f;
  float xip4 = 0.0f,  xip3 = 0.0f,  xip2 = 0.0f,  xip1 = 0.0f;
  for (int i=m-1; i>=0; --i) {
    float xi = x[i];
    float yi = n1*xip1+n2*xip2+n3*xip3+n4*xip4 -
                d1*yip1-d2*yip2-d3*yip3-d4*yip4;
    y[i] += yi;
    yip4 = yip3;  yip3 = yip2;  yip2 = yip1;  yip1 = yi;
    xip4 = xip3;  xip3 = xip2;  xip2 = xip1;  xip1 = xi;
  }
}

void DericheFilter::applyXN(int nd, const vector< vector<float> >& x, vector< vector<float> >& y) 
{	 
  assert(&x != &y);    
  int m2 = y.size();   //  length;
  int m1 = y[0].size();// length;
  float n0 = _n0[nd],  n1 = _n1[nd],  n2 = _n2[nd],  n3 = _n3[nd];
  float d1 = _d1[nd],  d2 = _d2[nd],  d3 = _d3[nd],  d4 = _d4[nd];

  vector <float> yim4(m1);
  vector <float> yim3(m1);
  vector <float> yim2(m1);
  vector <float> yim1(m1);
  vector <float> xim4(m1);
  vector <float> xim3(m1);
  vector <float> xim2(m1);
  vector <float> xim1(m1);
  vector <float> yi(m1);
  vector <float> xi(m1);
  for (int i2=0; i2<m2; ++i2) {
    vector<float> x2 = x[i2];
    vector<float> y2 = y[i2];
    for (int i1=0; i1<m1; ++i1) {
      xi[i1] = x2[i1];
      yi[i1] = n0*xi[i1]+n1*xim1[i1]+n2*xim2[i1]+n3*xim3[i1]
                        -d1*yim1[i1]-d2*yim2[i1]-d3*yim3[i1]-d4*yim4[i1];
      y2[i1] = yi[i1];
    }
    vector<float> yt = yim4;
    yim4 = yim3;
    yim3 = yim2;
    yim2 = yim1;
    yim1 = yi;
    yi = yt;
    vector<float> xt = xim3;
    xim3 = xim2;
    xim2 = xim1;
    xim1 = xi;
    xi = xt;
  }
  n1 = n1-d1*n0;
  n2 = n2-d2*n0;
  n3 = n3-d3*n0;
  float n4 = -d4*n0;
  if (nd%2!=0) {
    n1 = -n1;  n2 = -n2;  n3 = -n3;  n4 = -n4;
  }
  vector<float> yip4 = yim4;
  vector<float> yip3 = yim3;
  vector<float> yip2 = yim2;
  vector<float> yip1 = yim1;
  vector<float> xip4 = xim4;
  vector<float> xip3 = xim3;
  vector<float> xip2 = xim2;
  vector<float> xip1 = xim1;
  for (int i1=0; i1<m1; ++i1) {
    yip4[i1] = 0.0f;
    yip3[i1] = 0.0f;
    yip2[i1] = 0.0f;
    yip1[i1] = 0.0f;
    xip4[i1] = 0.0f;
    xip3[i1] = 0.0f;
    xip2[i1] = 0.0f;
    xip1[i1] = 0.0f;
  }
  for (int i2=m2-1; i2>=0; --i2) {
    vector<float> x2 = x[i2];
    vector<float> y2 = y[i2];
    for (int i1=0; i1<m1; ++i1) {
      xi[i1] = x2[i1];
      yi[i1] = n1*xip1[i1]+n2*xip2[i1]+n3*xip3[i1]+n4*xip4[i1] -
                d1*yip1[i1]-d2*yip2[i1]-d3*yip3[i1]-d4*yip4[i1];
      y2[i1] += yi[i1];
    }
    vector<float> yt = yip4;
    yip4 = yip3;
    yip3 = yip2;
    yip2 = yip1;
    yip1 = yi;
    yi = yt;
    vector<float> xt = xip4;
    xip4 = xip3;
    xip3 = xip2;
    xip2 = xip1;
    xip1 = xi;
    xi = xt;
  }
}

/**
  * Makes Deriche's numerator and denominator coefficients.
  */
void DericheFilter::makeND(double sigma) 
{
  _n0.resize(3);
  _n1.resize(3);
  _n2.resize(3);
  _n3.resize(3);
  _d1.resize(3);
  _d2.resize(3);
  _d3.resize(3);
  _d4.resize(3);

  // For 0th, 1st, and 2nd derivatives, ...
  for (int i=0; i<3; ++i) {
    double n0 = (i%2==0)?a0[i]+c0[i]:0.0;
    double n1 = exp(-b1[i]/sigma) * (
                  c1[i]*sin(w1[i]/sigma) -
                  (c0[i]+2.0*a0[i])*cos(w1[i]/sigma)) +
                exp(-b0[i]/sigma) * (
                  a1[i]*sin(w0[i]/sigma) -
                  (2.0*c0[i]+a0[i])*cos(w0[i]/sigma));
    double n2 = 2.0*exp(-(b0[i]+b1[i])/sigma) * (
                  (a0[i]+c0[i])*cos(w1[i]/sigma)*cos(w0[i]/sigma) -
                  a1[i]*cos(w1[i]/sigma)*sin(w0[i]/sigma) -
                  c1[i]*cos(w0[i]/sigma)*sin(w1[i]/sigma)) +
                c0[i]*exp(-2.0*b0[i]/sigma) +
                a0[i]*exp(-2.0*b1[i]/sigma);
    double n3 = exp(-(b1[i]+2.0*b0[i])/sigma) * (
                  c1[i]*sin(w1[i]/sigma) -
                  c0[i]*cos(w1[i]/sigma)) +
                exp(-(b0[i]+2.0*b1[i])/sigma) * (
                  a1[i]*sin(w0[i]/sigma) -
                  a0[i]*cos(w0[i]/sigma));
    double d1 = -2.0*exp(-b0[i]/sigma)*cos(w0[i]/sigma) -
                  2.0*exp(-b1[i]/sigma)*cos(w1[i]/sigma);
    double d2 = 4.0*exp(-(b0[i]+b1[i])/sigma) *
                  cos(w0[i]/sigma)*cos(w1[i]/sigma) +
                exp(-2.0*b0[i]/sigma) +
                exp(-2.0*b1[i]/sigma);
    double d3 = -2.0*exp(-(b0[i]+2.0*b1[i])/sigma)*cos(w0[i]/sigma) -
                  2.0*exp(-(b1[i]+2.0*b0[i])/sigma)*cos(w1[i]/sigma);
    double d4 = exp(-2.0*(b0[i]+b1[i])/sigma);
    _n0[i] = (float)n0;
    _n1[i] = (float)n1;
    _n2[i] = (float)n2;
    _n3[i] = (float)n3;
    _d1[i] = (float)d1;
    _d2[i] = (float)d2;
    _d3[i] = (float)d3;
    _d4[i] = (float)d4;
  }
  scaleN(sigma);
}

/**
  * Scales numerator filter coefficients to normalize the filters.
  * For example, the sum of the 0th-derivative filter coefficients
  * should be 1.0. The scale factors are computed from finite-length
  * approximations to the impulse responses of the three filters.
  */
void DericheFilter::scaleN(double sigma) 
{
  int n = 1+2*(int)(10.0*sigma);
  vector<float> x(n);
  vector<float> y0(n);
  vector<float> y1(n);
  vector<float> y2(n);
  int m = (n-1)/2;
  x[m] = 1.0f;
  applyN(0,x,y0);
  applyN(1,x,y1);
  applyN(2,x,y2);
  vector<double> s(3);
  for (int i=0,j=n-1; i<j; ++i,--j) {
    double t = i-m;
    s[0] += y0[j]+y0[i];
    s[1] += sin(t/sigma)*(y1[j]-y1[i]);
    s[2] += cos(t*sqrt(2.0)/sigma)*(y2[j]+y2[i]);
  }
  s[0] += y0[m];
  s[2] += y2[m];
  s[1] *= sigma*exp(0.5);
  s[2] *= -(sigma*sigma)/2.0*exp(1.0);
  for (int i=0; i<3; ++i) {
    _n0[i] /= s[i];
    _n1[i] /= s[i];
    _n2[i] /= s[i];
    _n3[i] /= s[i];
  }
}

///////////////////////////////////////////////////////////////////////////
VanVlietFilter::VanVlietFilter(double sigma) 
{
  POLES.resize(3);
  POLES[0].push_back(Cdouble( 1.12075, 1.27788));
  POLES[0].push_back(Cdouble( 1.12075,-1.27788));
  POLES[0].push_back(Cdouble( 1.76952, 0.46611));
  POLES[0].push_back(Cdouble( 1.76952,-0.46611));
  POLES[1].push_back(Cdouble( 1.04185, 1.24034));
  POLES[1].push_back(Cdouble( 1.04185,-1.24034));
  POLES[1].push_back(Cdouble( 1.69747, 0.44790));
  POLES[1].push_back(Cdouble( 1.69747,-0.44790));
  POLES[2].push_back(Cdouble( 0.94570, 1.21064));
  POLES[2].push_back(Cdouble( 0.94570,-1.21064));
  POLES[2].push_back(Cdouble( 1.60161, 0.42647));
  POLES[2].push_back(Cdouble( 1.60161,-0.42647));
  makeG(sigma);
}

void VanVlietFilter::applyN(int nd, const vector<float>& x, vector<float>& y) 
{
  RecursiveGaussianFilter::checkArrays(x,y);
  assert(&x != &y);
  if (RecursiveGaussianFilter::sameArrays(x,y))
  {
    _g[nd][0][0].applyForward(x,y);
    _g[nd][0][1].accumulateReverse(x,y);
    _g[nd][1][0].accumulateForward(x,y);
    _g[nd][1][1].accumulateReverse(x,y);

  }
  else
  {
    _g[nd][0][0].applyForward(x,y);
    _g[nd][0][1].accumulateReverse(x,y);
    _g[nd][1][0].accumulateForward(x,y);
    _g[nd][1][1].accumulateReverse(x,y);
  }	
}

void VanVlietFilter::applyXN(int nd, const vector< vector<float> >& x, vector< vector<float> >& y) {
  RecursiveGaussianFilter::checkArrays(x,y);
  if (RecursiveGaussianFilter::sameArrays(x,y))
  {
    assert(&x != &y);
    _g[nd][0][0].apply2Forward(x,y);
    _g[nd][0][1].accumulate2Reverse(x,y);
    _g[nd][1][0].accumulate2Forward(x,y);
    _g[nd][1][1].accumulate2Reverse(x,y);
  }    
  else
  {
    _g[nd][0][0].apply2Forward(x,y);
    _g[nd][0][1].accumulate2Reverse(x,y);
    _g[nd][1][0].accumulate2Forward(x,y);
    _g[nd][1][1].accumulate2Reverse(x,y);
  }
}

void VanVlietFilter::makeG(double sigma) {
  _g.resize(3);
  for (int i=0;i<3; i++)
  {
    _g[i].resize(2);
    for (int j=0;j<2; j++)
    {
      _g[i][j].resize(2);
    }
  }
  
  // Loop over filters for 0th, 1st, and 2nd derivatives.
  for (int nd=0; nd<3; ++nd) {

    // Adjust the poles for the scale factor q.
    vector<Cdouble> poles = adjustPoles(sigma, POLES[nd]);

    // Filter gain.
    double gain = computeGain(poles);
    double gg = gain*gain;

    // Residues.
    Cdouble d0(poles[0]);
    Cdouble d1(poles[2]);
    Cdouble e0 = d0.inv();
    Cdouble e1 = d1.inv();
    Cdouble g0 = gr(nd,d0,poles,gg);
    Cdouble g1 = gr(nd,d1,poles,gg);

    // Coefficients for 2nd-order recursive filters.
    double a10 = -2.0*d0.r;
    double a11 = -2.0*d1.r;
    double a20 = d0.norm();
    double a21 = d1.norm();
    double b00,b01,b10,b11,b20,b21;

    // 0th- and 2nd-derivative filters are symmetric.
    if (nd==0 || nd==2) {
      b10 = g0.i/e0.i;
      b11 = g1.i/e1.i;
      b00 = g0.r-b10*e0.r;
      b01 = g1.r-b11*e1.r;
      b20 = 0.0;
      b21 = 0.0;
      _g[nd][0][0] = makeFilter(b00,b10,b20,a10,a20);
      _g[nd][1][0] = makeFilter(b01,b11,b21,a11,a21);
      b20 -= b00*a20;
      b21 -= b01*a21;
      b10 -= b00*a10;
      b11 -= b01*a11;
      b00 = 0.0;
      b01 = 0.0;
      _g[nd][0][1] = makeFilter(b00,b10,b20,a10,a20);
      _g[nd][1][1] = makeFilter(b01,b11,b21,a11,a21);

    // 1st-derivative filter is anti-symmetric.
    } else if (nd==1) {
      b20 = g0.i/e0.i;
      b21 = g1.i/e1.i;
      b10 = g0.r-b20*e0.r;
      b11 = g1.r-b21*e1.r;
      b00 = 0.0;
      b01 = 0.0;
      _g[nd][0][0] = makeFilter(b00,b10,b20,a10,a20);
      _g[nd][1][0] = makeFilter(b01,b11,b21,a11,a21);
      b20 = -b20;
      b21 = -b21;
      b10 = -b10;
      b11 = -b11;
      b00 = 0.0;
      b01 = 0.0;
      _g[nd][0][1] = makeFilter(b00,b10,b20,a10,a20);
      _g[nd][1][1] = makeFilter(b01,b11,b21,a11,a21);
    }
  }
}

Recursive2ndOrderFilter VanVlietFilter::makeFilter(
  double b0, double b1, double b2, double a1, double a2)
{
  return Recursive2ndOrderFilter(
    (float)b0,(float)b1,(float)b2,(float)a1,(float)a2);
}

    /**
     * Evaluates residue of G(z) for the n'th derivative and j'th pole.
     */
Cdouble VanVlietFilter::gr(int nd, Cdouble polej, const vector<Cdouble>& poles, double gain) {
  Cdouble pj = polej;
  Cdouble qj = pj.inv();
  Cdouble c1(1.0,0.0);
  Cdouble gz(c1);

  if (nd==1) {
    gz.timesEquals(c1.minus(qj));
    gz.timesEquals(c1.plus(pj));
    gz.timesEquals(pj);
    gz.timesEquals(0.5);
  } else if (nd==2) {
    gz.timesEquals(c1.minus(qj));
    gz.timesEquals(c1.minus(pj));
    gz.timesEquals(-1.0);
  }
  Cdouble gp(c1);
  int np = poles.size();
  for (int ip=0; ip<np; ++ip) {
    Cdouble pi = poles[ip];
    if (!pi.equals(pj) && !pi.equals(pj.conj()))
      gp.timesEquals(c1.minus(pi.times(qj)));
    gp.timesEquals(c1.minus(pi.times(pj)));
  }
  return gz.over(gp).times(gain);
}

vector<Cdouble> VanVlietFilter::adjustPoles(double sigma, const vector<Cdouble>& poles){	               
  // Simple search for scale factor q that yields the desired sigma.
  double q = sigma;
  double s = computeSigma(q,poles);
  for (int iter=0; abs(sigma-s)>sigma*1.0e-8; ++iter) {
    //System.out.println("sigma="+sigma+" s="+s+" q="+q);
    //Check.state(iter<100,"number of iterations less than 100");
    s = computeSigma(q,poles);
    q *= sigma/s;
  }

  // Adjust poles.
  int npole = poles.size();
  vector<Cdouble> apoles(npole);
  for (int ipole=0; ipole<npole; ++ipole) {
    Cdouble pi = poles[ipole];
    double a = pow(pi.abs(),2.0/q);
    double t = atan2(pi.i,pi.r)*2.0/q;
    apoles[ipole] = Cdouble::polar(a,t).inv();
  }
  return apoles;
}

double VanVlietFilter::computeGain(vector<Cdouble>&  poles) {
  int npole = poles.size();
  Cdouble c1(1.0,0.0);
  Cdouble cg(c1);
  for (int ipole=0; ipole<npole; ++ipole) {
    cg.timesEquals(c1.minus(poles[ipole]));
  }
  return cg.r;
}

double VanVlietFilter::computeSigma(double sigma, const vector<Cdouble>& poles) {
  int npole = poles.size();
  double q = sigma/2.0;
  Cdouble c1(1.0);
  Cdouble cs;
  for (int ipole=0; ipole<npole; ++ipole) {
    Cdouble pi = poles[ipole];
    double a = pow(pi.abs(),-1.0/q);
    double t = atan2(pi.i,pi.r)/q;
    Cdouble b = Cdouble::polar(a,t);
    Cdouble c = c1.minus(b);
    Cdouble d = c.times(c);
    cs.plusEquals(b.times(2.0).over(d));
  }
  return sqrt(cs.r);
}
